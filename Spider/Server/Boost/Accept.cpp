//
// Created by Victor Debray on 03/10/2017.
//
...
{
  auto sock = new Connection(*io_service_);

  std::function<void(const boost::system::error_code &)> f(std::bind(&Accept::onAccept, this,
                                                                     callback, std::placeholders::_1,
                                                                     std::shared_ptr<Connection>(sock)));
